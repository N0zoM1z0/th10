...
| Completed | `CSound::Pause @ 0x0044D5B0` | TH10 retains `.\src\core\zwave.cpp` source-path strings and the target body matches the DirectSound CSound layout used by neighboring Stop/Unpause/Reset methods. The natural SDK-style Pause implementation in `src/ZWave.cpp` builds as a 29-byte relocation-free normal COFF COMDAT and cold-replays all 29 target bytes exactly. Do not promote the adjacent Stop/Unpause/Reset methods from the same source text yet: their current normal-COFF probes remain 84/82, 41/41 register-different, and 62/61 because those target entries use private EDI/EAX receiver contexts. |
...
| Defer | ItemManager callback cluster `0x0041B8E0`, `0x0041BA00`, `0x0041BA30` | Raw re-audit proved three authored bodies in the old inventory gap, and they are now tracked as reviewed ItemManager candidates. `0x0041BA00` is a 48-byte registered update callback: with the verified `__stdcall` core and target-equivalent `((flags >> 2) | flags)` gate logic, both normal COFF and the item `/GL` context emit 42 bytes; target uniquely saves manager ECX into ESI and reuses ECX for the bit test. `0x0041BA30` is a 29-byte draw callback that moves ECX to EAX and tail-jumps to `0x0041B8E0`; a natural external-call form emits 31 bytes. Do not force these adapters independently: recover the real update/draw core source contexts first, because the target exposes their private interprocedural ABIs. |
