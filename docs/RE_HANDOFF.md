...
| Active | `BulletUpdateRuntime @ 0x00406240` / `BulletManagerView::UpdateBullets @ 0x004065C0` | TH10 disassembly after `AdvanceTransformProgram` repeatedly TESTs the live +0x43C transform flags rather than caching one copy: bit 1 reuses AL once, then 0x10/0x20/0x40/0x100/0x80 and later masks reload the field. Removing the maintained cache changes the selected real `UpdateBullets` `/GL /GS` candidate from 815/885 with 37/733 normalized comparable bytes to 853/885 with 43/733 (49 raw matched), cutting the size gap from 70 to 32 bytes. The manager loop remains 251/249 with 38/225; `AdvanceTransformProgram` remains 1,548/1,454 with 141/1,326 in this graph. These are context-scoped non-exact diagnostics. |
