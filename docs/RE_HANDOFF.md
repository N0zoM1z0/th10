...
Fresh bullet-transform work also mapped `BulletRuntimeView::AdvanceTransformProgram @ 0x00406D90` (1,454 target bytes). In the current real sound/despawn/timer support graph it emits 1,516 bytes; this supersedes the earlier fake-seam 1,520-byte diagnostic for this owner only. The comparison remains non-exact and normalization is incomplete.
...
| In progress | `EnemyCancelBulletRecord @ 0x00408030` | Fresh TH10 re-audit identifies the 199-byte helper shared by the bullet transform interpreter and CancelBulletPattern. The target uses private ESI=bullet. Replacing the older raw +0x3F8 timer stores with the typed `AnmVmTimerView::SetCurrent(0)` source shape produces a 199/199-byte candidate with 131/183 normalized comparable bytes under the real CancelBulletPattern /GL /GS caller graph; all 14 canonical exact EnemyEclDispatcher units still replay exact. This is source-present but not byte-exact. |
