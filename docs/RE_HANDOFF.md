...
| Active | `BulletUpdateRuntime @ 0x00406240` | The 885-byte per-bullet owner remains non-exact, but the latest exact-helper re-audit materially changes its physical baseline. Reconnecting canonical-exact `UpdateBoundaryBounce @ 0x00407BE0` through its private member seam, together with the horizontal/vertical wrap and exact State8 members, restores a target-sized 885/885 real split-TU `/GL /GS` contribution and raises agreement from the previous 86/729 checkpoint to 228/729 normalized comparable bytes (238 raw), complete normalization. The older external-stack BoundaryBounce seam is therefore superseded. |
