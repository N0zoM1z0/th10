...
| Defer | `0x0041BEB0` polar-vector helper, 30 bytes | Factory-attested caller review corrects the older single-caller note: the helper has two direct callers, `0x0041AFD0` and `0x0041BB00`. Its body still matches the completed polar helpers, but neither caller has a maintained source owner, so keep it source-unmapped until their shared object/module is established by TH10 evidence. |
...
| Defer | `0x0041BE80` score-cap helper, 46 bytes | This sibling is called only by source-unmapped `0x0041AFD0`. It uses the same object base `0x00474C40`, updates `this+0xC` by `amount / 10`, and caps at `99999`; the shared object evidence is stronger now, but name/field semantics and the production caller context still need target-backed recovery before mapping it. |
