...
| Active | `BulletUpdateRuntime @ 0x00406240` | The 885-byte per-bullet owner remains non-exact, but the production partition at its transform seam is now recovered in maintained source. `src/BulletRuntime.cpp`, `src/BulletManager.cpp`, ANM and Enemy remain /GL /GS; `BulletRuntimeView::AdvanceTransformProgram` moved to `src/BulletTransform.cpp` and is supplied as a normal non-/GL support object through the new `--normal-support` probe path. The real resolved mixed-profile graph is 885/885 with 498/729 normalized comparable bytes (532 raw), complete normalization, versus 228/729 when the transform definition is visible to LTCG. This preserves the real member symbol rather than an unresolved diagnostic anchor and is the new caller baseline. |
