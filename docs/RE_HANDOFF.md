...
| Defer | `CSound::Play @ 0x0044D440` | With RestoreBuffer/GetFreeBuffer/Reset present naturally in the same ZWave TU, the selected `/GL` contribution now has the exact 145-byte target extent. Exactness remains open because candidate entry uses ECX while target uses EAX, and FillBufferWithSound/SetVolume receiver setup retains the private-register difference. Reopen only with the real external Play caller context; do not force receiver registers in source. |
