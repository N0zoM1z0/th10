...
| Defer | `AnmRenderManagerView::PreloadAnm @ 0x00447280` | Target has 11 callers; `AsciiManagerView::Initialize @ 0x00401110` is one real same-TU context. Marking the manual `Sleep` declaration `dllimport` changes the `/GL` candidate from 80 bytes (38/69 comparable) to 81 bytes (68/69) with the target IAT field at `0x004660AC`. The only remaining byte difference is `JL` (`0x7C`) versus target `JS` (`0x78`) after `TEST`, with the same sign-test result. Keep the natural signed comparison; defer until new source-expression or compiler-lowering evidence explains this byte. Ten other callers remain source-unmapped. |
