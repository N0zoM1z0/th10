...
| Active | `BulletUpdateRuntime @ 0x00406240` | The 885-byte per-bullet owner remains non-exact. With `BulletRuntime.cpp`, `BulletManager.cpp`, `PlayerCollision.cpp`, ANM and Enemy under /GL /GS and `BulletTransform.cpp` as normal non-/GL support, the real linked collision and transform callees yield an 885/885 contribution with 727/729 normalized comparable bytes. Only two register-encoding bytes at +0x24F/+0x25B differ: TH10 loads `g_Player` into EDX and moves the position into ECX before calling 0x004266B0, while the candidate swaps ECX/EDX. The earlier 887/885, 498/729 result lacks the collision owner and is a different support graph. Equal size and this diagnostic do not grant exactness. |
| Active | `PlayerCollisionPositionView::CheckPlayerCollision @ 0x004266B0` | Factory-attested xrefs give two direct callers, BulletUpdateRuntime and EnemyRuntimeUpdate. TH10 passes position in ECX, Player in EDX and collision size in EAX; the 316-byte body tests the supplied rectangle for a hit, applies the Player state/gate and hit handler, then tests a 24-unit graze rectangle. Maintained `src/PlayerCollision.cpp` provides a real callee for the Bullet graph. Its own codegen is unproven; the descriptive member name does not establish original class or TU. |
...
| Active | `BulletRuntimeView::AdvanceTransformProgram @ 0x00406D90` | TH10-local re-audit supersedes the older ANM VM opcode-handler interpretation. The 1,454-byte target walks up to 18 0x18-byte transform records at runtime +0x464, updates active flags +0x43C/cursor +0x45C and nine 0x34-byte extension states from +0x614. The owner and its private angle resolver now live in `src/BulletTransform.cpp`; caller-side reconstruction shows this TU must remain outside the runtime /GL visibility domain. Its own body is still non-exact and needs a fresh dedicated replay under the recovered partition. |
