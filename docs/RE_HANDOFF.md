...
| Active | `BulletUpdateRuntime @ 0x00406240` | The 885-byte per-bullet owner remains non-exact. It now lives in `src/BulletRuntime.cpp` so the exact manager can preserve the retail cross-TU call seam. TH10 target evidence still requires live reloads of +0x43C transform flags after `AdvanceTransformProgram`, state 1/2/3 movement, collision/effect handling, sprite-bound culling, timer decrements and the ANM executor tail. Refresh the diagnostic from this split-TU baseline before reusing older same-TU size/agreement figures. |
