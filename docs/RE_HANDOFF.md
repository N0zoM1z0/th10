...
| Next | `EclVmContext::ReadInt @ 0x0044FDB0` | Three target callers are `0x0040E770`, `0x00412A00`, and `0x0044E1A0`. A selected `/GL /GS` EclVmHost + Enemy support graph emits the full 144-byte extent, but the typed-stack pop path chooses ECX in three instruction fields where target uses ESI; the `__ftol2` call displacement is a separate linkage field. Compare the call setup and register lifetimes across all three real callers before another source edit. A local `poppedValue` rewrite changed the candidate to 117 bytes and changed its private entry/return convention, so that variant is closed. |
| Defer | `AnmSetVmScriptIndexAndExecute @ 0x004496D0` | All three target callers are in `EnemyRuntimeUpdate` and pass `&enemy->managedVmIds[0]` plus a script index. Current semantic source candidate is in `src/AnmManager.cpp` and performs shared-manager `FindVm`, null return, then `SetAndExecuteScriptIdx`. Its free-helper `/GL` candidate is 42 bytes versus the 36-byte target (1/28 comparable bytes; three differing fields); a natural manager-member form was 40/36 (2/29). Target uses private EAX VM-id pointer plus one stack script argument and `RET 4`, so keep exactness unresolved until linked-image extent/call-ABI evidence improves. The adjacent `SetAndExecuteScriptIdx @ 0x0043E8B0` remains a separate 287-byte near match at 249/271 comparable bytes. |
| Defer | `PlayerCreate @ 0x00425020` | The selected natural `/GL` candidate is 66 bytes versus the 69-byte target (12/49 comparable bytes; five differing fields). Its direct target caller `0x00417870` still has no maintained source owner; map that path before changing this constructor. |
| Defer | `EnemyEclResourceView::LoadFile @ 0x0040CD20` | Selecting its real same-TU `EnemyManagerView::Initialize` caller leaves the `/GL` candidate at 97 bytes versus 95 (50/79 normalized comparable bytes; four linkages). Revisit after new boundary or dependency evidence. |
...
| Defer | `AnmRenderManagerView::ReleaseAnm @ 0x004477D0` | The 55-byte target uses private EBX manager and ESI index state. Its only direct caller, `0x0040BD80`, has no maintained source owner, so keep it unplaced until that Enemy ECL path is mapped. |
| Defer | `OnDrawHighPriority @ 0x00401520` | The candidate and target are both 10 bytes, but target saves EBX while the candidate saves EDI. Equal size is not a match; recover the callback caller graph before trying register-shaping edits. |
...
| Defer | `SetVmPendingInterrupt @ 0x00449210` | Its sole direct caller, `PlayerUpdateMovementAndOptions @ 0x004250B0`, sets ESI to one; the 54-byte target reads SI and returns with `RET 4`, while the current candidate has a different private ABI. Reopen with that call site and maintained owner in view. |
