...
| Next | `AnmRenderManagerView::PreloadAnm @ 0x00447280` | Target has 11 callers; `AsciiManagerView::Initialize @ 0x00401110` is one real same-TU context. Marking the manual `Sleep` declaration `dllimport` changes the `/GL` candidate from 80 bytes (38/69 comparable) to 81 bytes (68/69) with the target IAT field at `0x004660AC`. The only remaining byte difference is `JL` (`0x7C`) versus target `JS` (`0x78`) after `TEST`, with the same sign-test result. This is a focused remaining branch-lowering question: retain natural source and seek target-supported source-expression/compiler evidence; do not force a branch encoding. Ten other callers remain source-unmapped. |
