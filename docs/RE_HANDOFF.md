...
| Defer | FrontEndCursorView::Move @ 0x0044BEA0 | The same caller audit gives 15 direct callers, again dominated by maintained FrontEnd owners, and target establishes EAX=cursor plus one stack delta and RET 4. The natural source has the correct 128-byte contribution in the same /GL graph but only 78/128 raw bytes agree because register/control-flow scheduling differs. Keep the source mapping, but do not promote exactness until new lowering evidence appears. |
