...
| Defer | `SetVmPendingInterrupt @ 0x00449210` | Its sole direct caller is `PlayerUpdateMovementAndOptions @ 0x004250B0`; target sets ESI to one, reads SI, and returns `RET 4`. The current player source path calls `PlayerSetManagedVmDeleteState(&player->modeVmId, 1)` rather than explicitly routing to this ANM-manager helper, and its candidate ABI still differs. Map that source operation and ownership before changing the helper spelling or ABI. |
