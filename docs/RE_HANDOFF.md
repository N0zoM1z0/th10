...
| Active | `BulletUpdateRuntime @ 0x00406240` | The 885-byte per-bullet owner remains non-exact. Reconnecting the target-proven `UpdateHorizontalWrap @ 0x00407DA0` member through the real split-TU caller graph changes the selected `BulletRuntime.cpp + BulletManager.cpp + AnmManager.cpp + EnemyEclDispatcher.cpp /GL /GS` candidate from 861/885 and 68/733 normalized comparable bytes to target-sized 885/885 with 80/733 (82 raw), complete normalization. This is a physical-context checkpoint, not exactness. Existing eight canonical exact units sourced from BulletManager.cpp cold-replay 1355/1355 bytes after the change. |
