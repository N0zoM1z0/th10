...
| Active | `BulletUpdateRuntime @ 0x00406240` | The 885-byte per-bullet owner remains non-exact. With the recovered member seams visible to LTCG the current real split-TU `/GL /GS` graph is target-sized 885/885 with 228/729 normalized comparable bytes (238 raw). A controlled production-partition diagnostic keeps the original `AdvanceTransformProgram` member call but hides only its `0x00406D90` definition from LTCG; the runtime stays 885/885 and jumps to 498/729 normalized bytes (533 raw). Restoring the definition falls back to 228/729; explicit `__fastcall` member and `#pragma inline_depth(0)` variants also remain 228/729. This strongly points to translation-unit/profile visibility at the transform seam rather than caller semantics. Current tooling forces all support sources through the same `/GL` profile, so mixed-profile ownership still needs recovery/tool support before this can become a canonical claim. |
