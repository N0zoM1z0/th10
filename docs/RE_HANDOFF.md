...
| Next | `CMemoryPbgFile::Seek @ 0x004364F0` | Keep the natural virtual method in `src/ResFile.cpp`. Target uses `ECX this`, two stack arguments, and `RET 8`; its 107-byte body dispatches `FILE_BEGIN/CURRENT/END` and shares the current/end pointer update. The if-chain candidate was 125 bytes with 6/107 comparable bytes. A switch over the target seek origin emits 117 bytes with 12/107 comparable bytes in the selected linked probe. Single-result and shared-base refactors emitted 118 and 122 bytes, respectively, and were reverted. The switch remains a diagnostic candidate only; review additional source/control-flow evidence before claiming exactness. |
| Defer | `CMemoryPbgFile::Open @ 0x00436400` | Keep the virtual method in `src/ResFile.cpp`. Target calls 0x435800 with filename in ECX plus a stack null, then calls 0x4358E0 with filename in ECX. The helper owners and source ABI remain unknown; do not guess a declaration from Ghidra's provisional convention. The selected `/GL` probe with `PbgArchive.cpp` support emits 51 bytes versus 46 (8/38 comparable bytes). Reopen with source-level helper ABI or compiler evidence. |
...
| Defer | `GuiSetLivesDisplayCount @ 0x00413790` | Four direct code references come from three functions: calls at `0x004188C3`, `0x00418925`, and `0x00426A3F`, plus a tail jump at `0x00424E9C`. All pass the GuiView owner in EAX and count in ECX. Only `PlayerResetRuntimeState @ 0x00424D90` has a maintained caller source; `0x004188A0` and `0x004269D0` remain unmapped. The natural `src/Gui.cpp` body builds as an 84-byte `/GL` entry candidate versus 77 target bytes (2/77 comparable bytes). Keep exactness open; revisit with source-owner or compiler-lowering evidence rather than forcing the private register ABI. |
