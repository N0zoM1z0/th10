...
| Active | `BulletUpdateRuntime @ 0x00406240` / `BulletManagerView::UpdateBullets @ 0x004065C0` | TH10-local recovery maps the 885-byte per-bullet update and 249-byte 2,000-row manager loop. Fresh `/GL /GS` graph gives 815/885 with 37/733 normalized comparable bytes for the per-bullet owner and 251/249 with 38/225 for the manager loop. This graph also improves `SpawnBulletPattern` to 194/194 with 24/170 normalized bytes and `SpawnSingleBullet` to 1,504/1,413 with 213/1,293; `AdvanceTransformProgram` remains 1,548/1,454 with 141/1,326. These are context-scoped non-exact diagnostics. |
| Active | `BulletPositionView::IsOutsidePlayfield @ 0x00406160` | TH10 proves the same x87 comparison skeleton as the already-exact 0x004086B0 sibling, with width/height halved first and cull rectangle X [-192,192], Y [-64,448]. Current candidate is 115/109 with 9/85 normalized comparable bytes. |
