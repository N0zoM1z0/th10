...
| Active | `BulletUpdateRuntime @ 0x00406240` | The 885-byte per-bullet owner remains non-exact, but the split-TU baseline is now materially closer. Restoring target-shaped state-3/state-2/active switch layout plus vector expressions for the three-component position updates changes the real `BulletRuntime.cpp` + `BulletManager.cpp` `/GL /GS` candidate to 861/885 bytes with 68/733 normalized comparable bytes (84 raw), complete normalization. The same re-audit recovers three formerly unmapped private ESI members: relative direction 0x00407780 (336/338), absolute direction 0x004078E0 (333/335), and aimed direction 0x00407A30 (429/431). All three remain non-exact; continue from this source/ABI checkpoint rather than the older same-TU figures. |
