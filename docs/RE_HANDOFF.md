...
- Commit substantive checkpoints with the operator-required `gpt-web: ...` prefix.
  Git push is not available through the Factory environment.
...
Fresh 2026-09-25 Bullet re-audit now maps the surrounding spawn graph as well as `BulletRuntimeView::AdvanceTransformProgram @ 0x00406D90`. In the expanded graph, `SpawnSingleBullet @ 0x004067D0` emits 1,500/1,413 bytes with 74/1,293 normalized comparable bytes, `AdvanceTransformProgram` emits 1,548/1,454 with 141/1,326, and `SpawnBulletPattern @ 0x004073E0` emits 195/194 with 6/170; normalization is complete for all three. These figures supersede the older 1,516/1,520 transform-only diagnostics only for this expanded graph and are non-exact. The same audit maps the distinct 202-byte embedded-VM initializer at `0x00404F30`; direct-entry replay is 202/202 with 47/194 normalized comparable bytes.
...
| Active | `BulletUpdateRuntime @ 0x00406240` | The 885-byte per-bullet owner remains non-exact. Reconnecting the target-proven horizontal wrap, vertical wrap and state-8 members through the real split-TU caller graph moves the selected `BulletRuntime.cpp + BulletManager.cpp + AnmManager.cpp + EnemyEclDispatcher.cpp /GL /GS` frontier from 861/885 and 68/733 to target-sized 885/885 with 86/733 normalized comparable bytes (88 raw), complete normalization. The State8 seam alone moves the current 83/733 checkpoint to 86/733. Canonical manager, boundary-bounce and wrap-bounds units remain exact after this source change. |
...
| Active | `BulletRuntimeView::UpdateState8 @ 0x00407EF0` | TH10 target uses exStates[8], player draw-position aiming, exact `BulletAngleDifference @ 0x00408660`, two AddNormalizeAngle calls, polar velocity rebuild and the shared timer tick. The first maintained member replay emits 307/306 bytes with 31/258 normalized comparable bytes; connecting this private ESI seam improves the target-sized 885-byte runtime from 83/733 to 86/733. Source semantics are mapped; codegen is non-exact. |
| Active | `BulletRuntimeView::AdvanceTransformProgram @ 0x00406D90` | TH10-local re-audit supersedes the older ANM VM opcode-handler interpretation. The 1,454-byte target walks up to 18 0x18-byte transform records at runtime +0x464, updates active flags +0x43C/cursor +0x45C and nine 0x34-byte extension states from +0x614. The newly recovered real spawn graph (`0x4067D0 -> 0x406D90`, plus child-pattern `0x4073E0`) changes the selected diagnostic to 1,548/1,454 bytes with 141/1,326 normalized comparable bytes, complete normalization. Do not compare this number directly with the earlier transform-only support graphs. |
